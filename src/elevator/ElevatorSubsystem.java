...
public class ElevatorSubsystem implements Runnable, ElevatorEvents {
	...
	public ElevatorSubsystem(String name, int port, int startFloor, int schedulerPort, int maxFloor, int travelTime, int passengerTime, int doorTime, String host){
		...
		this.state = new ElevatorState(startFloor,startFloor, Direction.STAY, ElevatorCurrentStatus.STOP, ElevatorCurrentDoorStatus.OPEN, maxFloor,travelTime,passengerTime,doorTime);
	...
	private void handleElevatorStop(){
		...
		this.consoleOutput(RequestEvent.SENT, "Scheduler", "STOP at " + this.state.getCurrentFloor() + ".");
		this.sendToServer(request);
	}
	...
	private void handleElevatorOpenDoor(){
		...
		if(!this.doorFaultFlag) {
			...
			this.consoleOutput(RequestEvent.SENT, "Scheduler", "Doors are OPEN.");
	...
	private void handleElevatorCLOSEoor(){
		...
		if(!this.doorFaultFlag) {
			...
			this.consoleOutput(RequestEvent.SENT, "Scheduler", "Doors are CLOSE.");
