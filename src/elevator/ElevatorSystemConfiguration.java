...
/**
 * ElevatorSystemConfiguration loads and configures the elevator
 *
 */
public class ElevatorSystemConfiguration {
	public final static int DEFAULT_PACKET_SIZE = 1024;
	public final static boolean SEND_PACKET_EVENT = true;
	public final static boolean RECEIVE_PACKET_EVENT = false;
	private static String configFile = "resources/config.xml";
	private static Document configDocument = null;

	/**
	 * Reads the config file from project resources.
	 */
	...
	/**
	 * Method used to parse config.xml and return a map of configurations for
	 * multiple items. This will return a map containing the attributes for all
	 * instances of 'element'.
	 * 
	 * @param element the tag associated with an element to parse from config.xml
	 *                (ie. Elevator, Floor, Scheduler)
	 */
	static public HashMap<String, HashMap<String, String>> getConfigurations(String element) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			// For each 'element' node get a NamedNodeMap consisting of key value pairs of
			// all of this element's attributes
			node = nodeList.item(i);
			NamedNodeMap nm = node.getAttributes();

			// Iterate through each of the attributes and construct a map of key value pairs
			// for each
			...
			// Get the name of this element form the the config.xml and use it as key for
	...
	/**
	 * Method used to parse config.xml and return a map of configurations for a
	 * single item. This will return a map containing the attributes for all
	 * instances of 'element'.
	 * 
	 * @param element element parsed form the config.xml (ie. Scheduler, etc.)
	 * @return
	 */
	...
	/**
	 * Get all elevators configuration for.
	 * 
	 * @return - HashMap containing an entry for each elevator, where the key is the
	 *         elevator name and the value is the port (each elevator will listen at
	 *         this specified port)
	 */
